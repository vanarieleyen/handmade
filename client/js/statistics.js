...
function calc_moving_average(period) {
	var nums = [];
	return function(num) {
		nums.push(num);
...
function mean(dat) {
	...
	for (var total=0, i=0; i < dat.length; i++)
		total += parseInt(dat[i]);
	return total/i;
}

// calculate Cp (process capability)
// hoe hoger hoe minder variatie in de samples
...
// hoe hoger hoe meer de samples aan de target voldoen
...
jStat.extend({
	...
	// mean value of an array (average)
	mean : function(arr) {
	...
	// flag indicates population vs sample
	// Example: jStat.stdev([4,5,9,7,5,3,4], true)
