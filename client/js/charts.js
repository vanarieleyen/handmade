...
function mini_chart(element, soort) {
	switch (soort) {
		case "length":
...
function draw_chart(arg) {
	...
	var defaults = {	
    	start: 	$('#eval #start_date').val(),	// begin tijdsperiode
		end: 		$('#eval #end_date').val(),
		product: $('#eval #product').val(),
		machine: $('#eval #machine').val(),
		station: $('#eval #station').val(),
		...
		ofwhat: 	$('#charts #chart1 option:selected').text().toLowerCase(),		// the selected raw data of chart1-selectbox
		filter: 	$('#eval #filterdata input:checked').val()		// 0=workfloor data, 1=inspectors data
	};
	var arg = extend(defaults, arg); // fill with defaults when no arguments are passed
	
	switch ($.jStorage.get("lasttab")) {
		case "#eval_penalties":	
					eval_penalties();
					break;
		case "#eval_machines":
					// get machines and fill selectbox
					$('#eval_machines #machinebox').empty();
					$.get('ajax/get_evalmachines.php?start='+arg.start+'&end='+arg.end+'&filter='+arg.filter, 
						function(data) {$('#eval_machines #machinebox').append(data);	});
				
					var machines = Array();
					$('#eval_machines #machinebox').children().each(function () {
						machines.push(Array($(this).val(), $(this).attr('table')));
					});
					machineComparisonChart('#eval_machines #overview', machines, arg.start, arg.end, arg.filter);	
					
					var machine = Array($('#eval_machines #machinebox').val(), $("#eval_machines #machinebox option:selected").attr('table') );
					var soort = $('#eval_machines #type input:checked').val();
					machineEvaluationChart('#eval_machines #graph1', machine, soort, arg.start, arg.end, arg.filter);	
					break;
	}
	...
	switch (arg.what1) {
		...
		case "carton":
		case "alldefects":
				$('#eval #filterdata [value=1]').prop('checked', true);
				arg.filter = 1;
				abcdefectsChart('#charts #graph1', arg.what1, arg.type1, arg.start, arg.end, arg.product, arg.machine, arg.station, arg.filter);
				break;
		case "penalties":
				penaltiesChart('#charts #graph1', arg.what1, arg.start, arg.end, arg.product, arg.machine, arg.station, arg.filter);
				break;
		default:		
			switch (arg.type1) {
				case 'Raw':			rawChart('#charts #graph1', arg.what1, arg.start, arg.end, arg.product, arg.machine, arg.station, arg.filter);
										break;
				case 'Average':	averageChart('#charts #graph1', arg.what1, arg.start, arg.end, arg.product, arg.machine, arg.station, arg.filter);
										break;
				case 'Deviation':	deviationChart('#charts #graph1', arg.what1, arg.start, arg.end, arg.product, arg.machine, arg.station, arg.filter);
										break;
				case 'Variance':	variationChart('#charts #graph1', arg.what1, arg.start, arg.end, arg.product, arg.machine, arg.station, arg.filter);
										break;
				case 'Cpk':			cpcpkChart('#charts #graph1', arg.what1, arg.start, arg.end, arg.product, arg.machine, arg.station, arg.filter);
										break;
				case 'Dist':		distributionChart('#charts #graph1', arg.what1, arg.start, arg.end, arg.product, arg.machine, arg.station, arg.filter);
										break;
			}
			break;
	}
	...
	switch (arg.what2) {
		...
		case "alldefects":
				$('#eval #filterdata [value=1]').prop('checked', true);
				arg.filter = 1;
				abcdefectsChart('#charts #graph2', arg.what2, arg.type2, arg.start, arg.end, arg.product, arg.machine, arg.station, arg.filter);
				break;
		case "penalties":
				penaltiesChart('#charts #graph2', arg.what2, arg.start, arg.end, arg.product, arg.machine, arg.station, arg.filter);
				break;
		default:	
			switch (arg.type2) {
				case 'Raw':			rawChart('#charts #graph2', arg.what2, arg.start, arg.end, arg.product, arg.machine, arg.station, arg.filter);
										break;
				case 'Average':	averageChart('#charts #graph2', arg.what2, arg.start, arg.end, arg.product, arg.machine, arg.station, arg.filter);
										break;
				case 'Deviation':	deviationChart('#charts #graph2', arg.what2, arg.start, arg.end, arg.product, arg.machine, arg.station, arg.filter);
										break;
				case 'Variance':	variationChart('#charts #graph2', arg.what2, arg.start, arg.end, arg.product, arg.machine, arg.station, arg.filter);
										break;
				case 'Cpk':			cpcpkChart('#charts #graph2', arg.what2, arg.start, arg.end, arg.product, arg.machine, arg.station, arg.filter);
										break;
				case 'Dist':		distributionChart('#charts #graph2', arg.what2, arg.start, arg.end, arg.product, arg.machine, arg.station, arg.filter);
										break;
			}
			break;
	}
	
	$(".OFWHAT").text(" ("+arg.ofwhat+")");
	$.getJSON('ajax/get_summary.php?start='+arg.start+'&end='+arg.end+'&what='+arg.what1+'&product='+arg.product+'&machine='+arg.machine+
							'&station='+arg.station+'&filter='+arg.filter, 
		function(data) {
			$('#charts ._samples').text(data.samples);
			$('#charts ._MEASUREMENTS').text(data.total);
			$('#charts ._MIN').text(data.min);
			$('#charts ._MAX').text(data.max);
			$('#charts ._AVERAGE').text(data.avg);
			$('#charts ._DEVIATION').text(data.dev);
			$('#charts ._VARIANCE').text(data.var);
			$('#charts ._OUTSPEC').text(sprintf("%s (%s%%)", data.out, data.out_pct));
			$('#charts ._CP').text(data.cp);
			$('#charts ._CPK').text(data.cpk);		
	});
}
...
function rawChart(chart, table, start, end, product, machine, station, filter) {	
	var ytext, field, eff = [];

	switch(table) {
		case 'weight': 		ytext = LABELS[42][$.jStorage.get("lang")];
									field = 'weight';
									break;
		case 'ventilation': 	ytext = LABELS[516][$.jStorage.get("lang")];
									field = 'vent';
									break;
		case 'pd': 				ytext = LABELS[146][$.jStorage.get("lang")];
									field = 'pd';
									break;
		case 'length': 		ytext = LABELS[102][$.jStorage.get("lang")];
									field = 'len';
									break;
		case 'hardness': 		ytext = LABELS[520][$.jStorage.get("lang")];
									field = 'hd';
									break;
		case 'circumference':ytext = LABELS[515][$.jStorage.get("lang")];
									field = 'circ';
									break;
	}
...
// average chart
function averageChart(chart, table, start, end, product, machine, station, filter) {	
	var ytext, field;

	if (product == null)		return;	// without a product there are no charts
	
	switch(table) {
		case 'weight': 		ytext = sprintf("%s (%s)", LABELS[42][$.jStorage.get("lang")], LABELS[158][$.jStorage.get("lang")]);
									field = 'weight';
									break;
		case 'ventilation': 	ytext = sprintf("%s (%s)", LABELS[516][$.jStorage.get("lang")], LABELS[158][$.jStorage.get("lang")]);
									field = 'vent';
									break;
		case 'pd': 				ytext = sprintf("%s (%s)", LABELS[146][$.jStorage.get("lang")], LABELS[158][$.jStorage.get("lang")]);
									field = 'pd';
									break;
		case 'length': 		ytext = sprintf("%s (%s)", LABELS[102][$.jStorage.get("lang")], LABELS[158][$.jStorage.get("lang")]);
									field = 'len';
									break;
		case 'hardness': 		ytext = sprintf("%s (%s)", LABELS[520][$.jStorage.get("lang")], LABELS[158][$.jStorage.get("lang")]);
									field = 'hd';
									break;
		case 'circumference':ytext = sprintf("%s (%s)", LABELS[515][$.jStorage.get("lang")], LABELS[158][$.jStorage.get("lang")]);
									field = 'circ';
									break;
	}
...
// std. deviation chart
function deviationChart(chart, table, start, end, product, machine, station, filter) {	
	var ytext, field, eff = [];

	if (product == null)		return;	// without a product there are no charts

	switch(table) {
		case 'weight': 		ytext = sprintf("%s (%s)", LABELS[42][$.jStorage.get("lang")], LABELS[517][$.jStorage.get("lang")]);
									break;
		case 'ventilation': 	ytext = sprintf("%s (%s)", LABELS[516][$.jStorage.get("lang")], LABELS[517][$.jStorage.get("lang")]);
									break;
		case 'pd': 				ytext = sprintf("%s (%s)", LABELS[146][$.jStorage.get("lang")], LABELS[517][$.jStorage.get("lang")]);
									break;
		case 'length': 		ytext = sprintf("%s (%s)", LABELS[102][$.jStorage.get("lang")], LABELS[517][$.jStorage.get("lang")]);
									break;
		case 'hardness': 		ytext = sprintf("%s (%s)", LABELS[520][$.jStorage.get("lang")], LABELS[517][$.jStorage.get("lang")]);
									break;
		case 'circumference':ytext = sprintf("%s (%s)", LABELS[515][$.jStorage.get("lang")], LABELS[517][$.jStorage.get("lang")]);
									break;
	}
...
// variance chart
function variationChart(chart, table, start, end, product, machine, station, filter) {	
	var ytext, field, eff = [];

	if (product == null)		return;	// without a product there are no charts
	
	switch(table) {
		case 'weight': 		ytext = sprintf("%s (%s)", LABELS[42][$.jStorage.get("lang")], LABELS[518][$.jStorage.get("lang")]);
									break;
		case 'ventilation': 	ytext = sprintf("%s (%s)", LABELS[516][$.jStorage.get("lang")], LABELS[518][$.jStorage.get("lang")]);
									break;
		case 'pd': 				ytext = sprintf("%s (%s)", LABELS[146][$.jStorage.get("lang")], LABELS[518][$.jStorage.get("lang")]);
									break;
		case 'length': 		ytext = sprintf("%s (%s)", LABELS[102][$.jStorage.get("lang")], LABELS[518][$.jStorage.get("lang")]);
									break;
		case 'hardness': 		ytext = sprintf("%s (%s)", LABELS[520][$.jStorage.get("lang")], LABELS[518][$.jStorage.get("lang")]);
									break;
		case 'circumference':ytext = sprintf("%s (%s)", LABELS[515][$.jStorage.get("lang")], LABELS[518][$.jStorage.get("lang")]);
									break;
	}
